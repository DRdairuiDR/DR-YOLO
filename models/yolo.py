...
class IDetect_Decoupled(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            x_cls = self.im_cls[i](self.m_cls[i](self.ia_cls[i](x[i]))).view(bs, self.na, self.nc, ny, nx).permute(0, 1,
                                                                                                                   3, 4,
                                                                                                                   2).contiguous()
            x_reg_conf = self.m_reg_conf[i](x[i])
            x_reg = self.im_reg[i](self.m_reg[i](self.ia_reg[i](x_reg_conf))).view(bs, self.na, 4, ny, nx).permute(0, 1,
                                                                                                                   3, 4,
                                                                                                                   2).contiguous()
            x_conf = self.im_conf[i](self.m_conf[i](self.ia_conf[i](x_reg_conf))).view(bs, self.na, 1, ny, nx).permute(
    ...
    def fuse(self):
        ...
        for i in range(len(self.m_cls)):
            ...
            self.m_cls[i][-1].bias += torch.matmul(self.m_cls[i][-1].weight.reshape(c1, c2),
                                                   self.ia_cls[i].implicit.reshape(c2_, c1_)).squeeze(1)
        ...
        for i in range(len(self.m_reg)):
            ...
            self.m_reg[i].bias += torch.matmul(self.m_reg[i].weight.reshape(c1, c2),
                                               self.ia_reg[i].implicit.reshape(c2_, c1_)).squeeze(1)
        ...
        for i in range(len(self.m_conf)):
            ...
            self.m_conf[i].bias += torch.matmul(self.m_conf[i].weight.reshape(c1, c2),
                                                self.ia_conf[i].implicit.reshape(c2_, c1_)).squeeze(1)
...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, IDetect_Decoupled):
            ...
            self._initialize_biases_IDetect_Decoupled()  # only run once
    ...
    def _initialize_biases_IDetect_Decoupled(self, cf=None):
        m = self.model[-1]  # Detect() module
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, E_ELAN, E_ELAN_H, DWConv, GhostConv, RepConv, RepConv_OREPA,
            ...
            gg = []
            for g in args:
                    gg.append(make_divisible(g * gw, 8))
            args = gg
            ...
            c2 = ch[f] // 2
        elif m in [Detect, IDetect, IAuxDetect, IBin, IKeypoint, IDetect_Decoupled]:
